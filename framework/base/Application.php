...
//--------------------------------------------------------------------
    ...
    public function createController($id, $route)
    {
        ...
        $file = $base . '/' .$prefix . $className . '.php';
        if (is_file($file)) {
    ...
    public function createUrl($route, $params = [])
    {
        ...
        $url = ($this->showScriptName ? $request->getScriptUrl() : $this->baseUrl ) . '/' . rtrim($route, '/');
        if (!empty($params) && ($query = http_build_query($params)) !== '') {
            $url .= '?' . $query;
        }
        return $url;
    }
   function singkatAngka($n, $presisi) {
	if ($n < 900) {
