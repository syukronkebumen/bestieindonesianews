   ...
   public function lists($cat = null){
    ...
    public function simpan(){
			...
			if(	isset($file['tmp_name']) && !empty($file['tmp_name']) ){
			 if ( $data['kategori'] == '2'  ){
			     $img = $this->uploader(750,540);
                 $data['image_url'] 		= $img;
			     //$name = $file["name"];
			     //move_uploaded_file($file['tmp_name'],Dee::$app->basePathe.'/uploads/slide/'. $name);
                // $data['image_url'] 		= $name;
			  }elseif( $data['kategori'] == '1'  ){
		        $img = $this->uploader(1920,782);
                $data['image_url'] 		= $img;
                }else{
		        $img = $this->uploader(960,540);
                $data['image_url'] 		= $img;
                }
			...
			if(	isset($data['image_url']) && !empty($data['image_url']) ){
                ...
                if($data['id'] > 1) {
                    $simpan = \Dee::$app->db->update("galery",$data,['id' => $data['id']]);
                     ...
                     $simpan = \Dee::$app->db->insert("galery",$data);
                }
				if($simpan){
			        $response['status'] = "success";
					$response['message'] = "Image berhasil di upload";
